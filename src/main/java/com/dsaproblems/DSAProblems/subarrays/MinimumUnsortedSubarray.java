...
public class MinimumUnsortedSubarray {
    ...
    private static List<Integer> findMinimumUnsortedSubarrayv1(List<Integer> input) {
        ...
        if (startBreakPoint == -1 || endBreakPoint == -1) return new ArrayList<>(Arrays.asList(-1));
        int max = -1; //non negative integers
        ...
        int start = -1;
        int end = -1;
        for (int i = 0; i <= startBreakPoint; i++) {
            if (min < input.get(i)) {
                start = i;
                break;
            }
        }
        for (int i = len - 1; i >= endBreakPoint; i--) {
            if (max > input.get(i)) {
                end = i;
                break;
            }
