...
//    }
    ...
    public static void main(String[] args) {
        ...
        // System.out.println(equlibriumIndexv1(A));
        System.out.println(equlibriumIndexv2(A));
        System.out.println(equlibriumIndexv3(A));
        System.out.println(equlibriumIndexv4(A));
    }

    // working code, no enhancement in performance
    private static int equlibriumIndexv4(ArrayList<Integer> input) {
        int len = input.size();
        if (len == 1) {
            return 0;
        }
        ...
        for (int i = 0; i < input.size(); i++) {
            int left = i == 0 ? 0 : prefix.get(i - 1);
            int right = i == len - 1 ? 0 : prefix.get(len - 1) - prefix.get(i);
            if (left == right) {
                return i;
            }
        }
    ...
    // better as one prefix array
    private static int equlibriumIndexv3(ArrayList<Integer> input) {
        int len = input.size();
        if (len == 1) {
            return 0;
        }
    ...
    // working but bad performance since two prefix arrays
    private static int equlibriumIndexv2(ArrayList<Integer> input) {
        int len = input.size();
        if (len == 1) {
            return 0;
        }
        ...
        leftprefix.add(input.get(0));
        rightprefix.add(input.get(len - 1));
        for (int i = 1; i < len; i++) {
            leftprefix.add(leftprefix.get(i - 1) + input.get(i));
            rightprefix.add(0, rightprefix.get(0) + input.get(len - 1 - i));
        }
        for (int i = 0; i < len; i++) {
            if (i == 0) {
                if (rightprefix.get(i + 1).equals(0)) {
                    return 0;
                }
                continue;
            } else if (i == len - 1) {
                if (leftprefix.get(i - 1).equals(0)) {
                    return len - 1;
                }
                continue;
            } else {
                if (leftprefix.get(i - 1).equals(rightprefix.get(i + 1))) {
                    return i;
                }
            }
        }
    ...
    private static int equlibriumIndexv1(ArrayList<Integer> A) {
        int len = A.size();
        if (len == 1) {
            return 0;
        }
        ...
        int left = 0;
        int right = A.get(len - 1) - A.get(0);
        if (left == right) {
            return 0;
        }
        for (int i = 1; i < len; i++) {
            left = A.get(i - 1);
            right = A.get(len - 1) - A.get(i);
            if (left == right) {
                return i;
            }
        }
