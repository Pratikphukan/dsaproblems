...
//		return Math.max(Math.max(l, r),node.data);
    ...
    // 2^(h+1)-1
    // h = log(n+1)-1
    public ArrayList<Integer> iterativeLevelOrderTraversal(Node head) {
        ArrayList<Integer> ans = new ArrayList<>();
        if (head == null) {
            return ans;
        }
        ...
        q.add(head);
        while (!q.isEmpty()) {
            Node temp = q.remove(); // The element is removed from the beginning or head of the linked list
            ans.add(temp.data);
            if (temp.left != null) {
                q.add(temp.left);
            }
     ...
     */
    public ArrayList<ArrayList<Integer>> iterativeLevelOrderTraversal1(Node head) {
    ...
    public ArrayList<Integer> levelWiseSum(Node head) {
        ArrayList<Integer> ans = new ArrayList<Integer>();
        if (head == null) {
            return ans;
        }
