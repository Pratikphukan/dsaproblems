...
public class MishaAndCandies {
    ...
    // working code
    //new PriorityQueue<>(boxes) → O(n) vs O(n log n) build
    private static int noOfCandiesConsumedv2(List<Integer> boxes, int maxCandiesLikedByMisha) {
        Queue<Integer> minHeap = new PriorityQueue<>(boxes);
        int candiesConsumed = 0;
        while (!minHeap.isEmpty()) {
            int currBox = minHeap.poll();
            if (currBox > maxCandiesLikedByMisha) break;
            candiesConsumed += currBox / 2;
            if (minHeap.isEmpty()) break;
            int nextBox = minHeap.poll() + (currBox - currBox / 2);
            if (nextBox <= maxCandiesLikedByMisha) minHeap.offer(nextBox);
        }
