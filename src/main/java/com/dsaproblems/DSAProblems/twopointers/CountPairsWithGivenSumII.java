...
public class CountPairsWithGivenSumII {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findCountPairsWithGivenSumv6(input, targetSum));
        int[] A = {2, 3, 3, 5, 7, 7, 8, 9, 9, 10, 10};
        System.out.println(findCountPairsWithGivenSumv6(A, targetSum));
    }

    private static int findCountPairsWithGivenSumv6(int[] A, int targetSum) {
    ...
    private static int findCountPairsWithGivenSumv6(List<Integer> nums, int target) {
        int MOD = 1_000_000_007;
        Map<Integer, Integer> map = new HashMap<>();
        for (int num : nums) {
            map.put(num, map.getOrDefault(num, 0) + 1);
        ...
        for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
            int num = entry.getKey(), complement = target - entry.getKey();
            if (entry.getValue() > 0 && map.getOrDefault(complement, 0) > 0) {
                if (num == complement && entry.getValue() < 2) continue;
                if (num == complement) {
                    long freq = entry.getValue();
                    count = (count + (freq * (freq - 1) / 2) % MOD) % MOD;
                    entry.setValue(0);
                } else {
                    long freq1 = entry.getValue();
                    long freq2 = map.get(complement);
                    count = (count + (freq1 * freq2) % MOD) % MOD;
                    entry.setValue(0);
                    map.put(complement, 0);
