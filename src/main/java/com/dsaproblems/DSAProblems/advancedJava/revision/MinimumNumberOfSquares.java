...
public class MinimumNumberOfSquares {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(countOfNumbersWhoseSquareSumv2((int) Math.sqrt(97280), 97280));
    }

    //not working
    private static int countOfNumbersWhoseSquareSumv2(int n, int sum) {
        if (n == 1) return sum;
        if (sum == 0) return 0;
        int count = countOfNumbersWhoseSquareSumv2(n - 1, sum);
        if (sum - n * n >= 0) {
            count = Math.min(count,
                    1 + countOfNumbersWhoseSquareSumv2(n, sum - n * n));
        }
        return count;
    }
