...
public class SubsequenceSumProblem {
    ...
    //target will be a non negative integer
    ...
    //working code
    //he possible subset sums can be negative, positive, and unbounded in both directions (especially with negative numbers in the input)
    private static boolean isSubsequenceSumExistsv4(List<Integer> input, int target) {
        Set<Integer> sums = new HashSet<>();
        sums.add(0);//Start with a set containing only 0 (sum with no elements)
        for (int num : input) {
            Set<Integer> newSums = new HashSet<>(sums);
            for (int s : sums) {
                newSums.add(s + num); //For each number, add it to all existing sums to form new sums
            }
            sums = newSums;
        }
        return sums.contains(target); //At the end, check if the target sum exists in the set
    }
    ...
    private static boolean isSubsequenceSumExistsv5(List<Integer> input, int target, int sum, int len) {
        ...
        //2 possibilities: add the number at index or don't add the number at index
